...
export default function DeleteTimeSlots({ isOpen, onClose, data, doctID }) {
  ...
  const DeleteRole = async () => {
    ...
    try {
      setisLoading(true);
      const res = await DELETE(admin.token, "delete_timeslots", formData);
      setisLoading(false);
      if (res.response === 200) {
        showToast(toast, "success", "Time Slot Deleted!");
        queryClient.invalidateQueries(["time-slotes", doctID]);
        onClose();
      } else {
      ...
      <AlertDialogOverlay>
        <AlertDialogContent>
          <AlertDialogHeader fontSize="lg" fontWeight="semi-bold">
            Delete <b>{data?.day}</b> <b>{data.id}</b> OPD timeslot ?
          </AlertDialogHeader>
